...
-- ============================================================================
-- MIGRATION: FIX STOP_SETTINGS, TIMELINE COLUMNS, AND 01K ROUTE DATA
-- Run this ENTIRE block in your Supabase SQL Editor!
-- ============================================================================

-- ============================================================================
-- STEP 1: Add missing columns to routes table (for schedule/timeline)
-- ============================================================================
ALTER TABLE routes 
...
-- ============================================================================
-- STEP 2: Add missing columns to stop_settings (for distance/time display)
-- The route queries reference these columns but they didn't exist!
-- ============================================================================
ALTER TABLE stop_settings
...
-- ============================================================================
-- STEP 3: Backfill stop_settings distance/time from route_paths data
-- This copies the existing route_paths timing data into stop_settings
-- ============================================================================
UPDATE stop_settings ss
SET 
  estimated_time_minutes = rp.estimated_time_minutes,
  distance_meters = rp.distance_meters
FROM route_paths rp
WHERE ss.route_id = rp.route_id 
  AND ss.checkpoint_id = rp.checkpoint_id
  AND rp.direction = 'FORWARD';

-- ============================================================================
-- STEP 4: Create unique index on checkpoint name (prevents duplicates)
-- ============================================================================
CREATE UNIQUE INDEX IF NOT EXISTS idx_checkpoints_unique_name ON checkpoints(name);

-- ============================================================================
-- STEP 5: Insert 01K Route (Urgello - Parkmall)
-- ============================================================================
...
-- ============================================================================
-- STEP 6: Insert 01K Checkpoints (safe upsert - skips if name exists)
-- ============================================================================
...
-- ============================================================================
-- STEP 7: Insert 01K stop_settings WITH distance and time data
-- ============================================================================
...
INSERT INTO stop_settings (route_id, checkpoint_id, stop_order, stop_type, waiting_time_minutes, estimated_time_minutes, distance_meters) VALUES
((SELECT id FROM route_id_val), (SELECT id FROM cp1), 1, 'TERMINAL', 5, 0, 0),
((SELECT id FROM route_id_val), (SELECT id FROM cp2), 2, 'REGULAR', 2, 5, 600),
((SELECT id FROM route_id_val), (SELECT id FROM cp3), 3, 'REGULAR', 2, 8, 900),
((SELECT id FROM route_id_val), (SELECT id FROM cp4), 4, 'REGULAR', 2, 12, 1100),
((SELECT id FROM route_id_val), (SELECT id FROM cp5), 5, 'REGULAR', 2, 22, 3500),
((SELECT id FROM route_id_val), (SELECT id FROM cp6), 6, 'TERMINAL', 5, 30, 2200)
...
SELECT 
  ...
  ss.stop_order,
  ss.stop_type,
  ss.estimated_time_minutes,
  ss.distance_meters,
  c.coordinates
FROM stop_settings ss
JOIN routes r ON r.id = ss.route_id
JOIN checkpoints c ON c.id = ss.checkpoint_id
ORDER BY r.route_code, ss.stop_order;
